...
function initAddAdminSection() {
  ...
  console.log("addAdminBtn element:", addAdminBtn);
  
  if (addAdminBtn) {
    addAdminBtn.addEventListener("click", addNewAdmin);
    console.log("Event listener attached to addAdminBtn");
  } else {
...
function addNewAdmin() {
  console.log("addNewAdmin function called");
  ...
  if (existingUserEmail) {
    ...
    alert(`New admin account "${firstName} ${lastName}".trim() created successfully`);
  } else {
    alert("Please select an existing user OR enter all new admin details (Username, Email, and Password)");
    return;
  }
  ...
  if (!admins.some(admin => admin.email === emailToPromote)) {
    ...
    alert(`User "${firstName} ${lastName}" (${emailToPromote}) has been promoted to administrator`);
  } else {
    alert(`User "${firstName} ${lastName}" (${emailToPromote}) is already an admin`);
  }
  ...
  loadData(); // This reloads allUsers and admins from localStorage
  console.log("After loadData - allUsers:", allUsers.length);
  
  // Check if the new user is in the array
  const newUser = allUsers.find(user => user.email === emailToPromote);
  console.log("Found new user in allUsers:", newUser);
  
  // Update all UI sections
  loadDashboard();
  loadUsers(); // This should update the users table
  loadAddAdminForm(); // Refresh the dropdown
  
  // Go to Users section to see the new admin
  showSection("users");
}
